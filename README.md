...
| 7/17 ~ 7/23 | 4주차 | 테스트셋 30~50건 수집 · Ground Truth 데이터 정리 · F1 정확도 측정 후 프롬프트 개선·재측정 · Docker 배포 |
...
팀 직접 라벨링 대신 공정거래위원회 의결서·재결서를 Ground Truth로 사용한다.

| 라벨 | 소스 |
|------|------|
| High (양성 샘플) | 공정위 의결서·재결서 게시판에서 "불공정약관 + 시정명령"으로 검색된 실제 조항 원문 |
| 기타 (음성 샘플) | 인덱싱된 FTC 표준계약서 192개의 정상 조항 |

수집 경로: 공정위 홈페이지 → 심결·법령 → 의결서·재결서  
필터 조건: 대표위반유형: 불공정약관 / 대표조치유형: 시정명령

### 평가 항목

| 평가 항목 | 목표 | 결과 |
|-----------|------|------|
| High 리스크 분류 정확도 (F1) | 85% 이상 | 0.94 |
| 조항 파싱 정확도 | 90% 이상 | 측정 예정 |
| 대안 문구 적절성 (3점 척도) | 2.5 이상 | 측정 예정 |
| 분析 처리 시간 (10p 기준) | 60초 이내 | 측정 예정 |

> Ground Truth: 공정거래위원회 의결서·재결서 기반 (불공정약관+시정명령 필터)
...
**같이**
- 테스트 계약서 30~50건 수집 · Ground Truth 데이터 정리 · 수치 확인
